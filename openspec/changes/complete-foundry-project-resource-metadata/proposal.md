...
- [`src/utils/armTemplate.ts`](/mnt/c/Users/lizha/OneDrive/Develop/Projects/Zeta/ai-foundry-manager/src/utils/armTemplate.ts) currently generates the Foundry project resource with the right type and dependency, but without `location`, `displayName`, or `description`.
...
- Risk: The code could keep creating a project resource that exists structurally but is still missing required metadata.
  - Mitigation: add exact template assertions for `location`, `displayName`, and `description`.
