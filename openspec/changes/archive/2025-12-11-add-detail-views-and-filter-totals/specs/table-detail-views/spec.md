...
### Requirement: Summary Data Calculation
原有合计行计算逻辑 MUST 修改为基于筛选后数据。

#### Scenario: ModelOverviewTable 合计计算
Given ModelOverviewTable 组件接收 filteredAccounts
When 组件计算 summaryData
Then SHALL 基于 filteredAccounts 计算各项统计值
And SHALL NOT 基于原始 accounts 数组

#### Scenario: ModelStatisticsTable 合计计算
Given ModelStatisticsTable 组件接收 filteredModelStates
When 组件计算 modelSummaryData
Then SHALL 基于 filteredModelStates 计算各项统计值
And SHALL NOT 基于原始 modelStates 数组
